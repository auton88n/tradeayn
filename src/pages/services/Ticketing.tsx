...
const Ticketing = () => {
  ...
  const t = {
    ...
    heroDescription: language === 'ar' ? 'نظام متكامل لإدارة الفعاليات: بيع التذاكر إلكترونياً، إرسال رموز QR للهواتف، والتحقق الفوري عند الدخول.' : language === 'fr' ? 'Système complet de gestion d\'événements : vente de billets en ligne, codes QR mobiles et validation instantanée à l\'entrée.' : 'Complete event management: online ticket sales, mobile QR codes, and instant validation at entry.',
    ...
    aiCustomerServiceDesc: language === 'ar' ? 'روبوت محادثة ذكي يعمل على مدار الساعة للرد على استفسارات الحضور حول الفعاليات والتذاكر ومعلومات المكان' : language === 'fr' ? 'Chatbot intelligent disponible 24h/24 répondant aux questions des participants sur les événements, billets et informations du lieu' : '24/7 AI chatbot answers attendee questions about events, tickets, and venue info',
    aiVipInvitations: language === 'ar' ? 'دعوات VIP الذكية' : language === 'fr' ? 'Invitations VIP IA' : 'AI VIP Invitations',
    aiVipInvitationsDesc: language === 'ar' ? 'إنشاء دعوات VIP مخصصة تلقائياً بناءً على سجل الضيوف وتفضيلاتهم السابقة' : language === 'fr' ? 'Génération automatique d\'invitations VIP personnalisées basées sur l\'historique et les préférences des invités' : 'Auto-generate personalized VIP invitations based on guest history and preferences',
    aiMarketing: language === 'ar' ? 'حملات التسويق الذكية' : language === 'fr' ? 'Campagnes Marketing IA' : 'AI Marketing Campaigns',
    aiMarketingDesc: language === 'ar' ? 'حملات بريد إلكتروني ورسائل نصية ذكية تُحسّن التوقيت والمحتوى لتحقيق أقصى مبيعات للتذاكر' : language === 'fr' ? 'Campagnes email et SMS intelligentes optimisant le timing et le contenu pour maximiser les ventes de billets' : 'Smart email and SMS campaigns that optimize timing and messaging for maximum ticket sales',
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [{
      ...
      position: 1,
      name: 'Home',
      item: 'https://ayn-insight-forge.lovable.app'
    }, {
      '@type': 'ListItem',
      position: 2,
      name: 'Smart Ticketing System',
      item: 'https://ayn-insight-forge.lovable.app/services/ticketing'
    }]
  };
  return <>
      <SEO title={`${t.title} | AYN`} description={t.heroDescription} canonical="/services/ticketing" keywords="ticketing system, QR code tickets, event management, mobile scanning, ticket validation" jsonLd={breadcrumbSchema} />
          ...
          <DialogContent className="sm:max-w-md">
            ...
            {isSuccess ? <div className="text-center py-8">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="message" rows={3} value={formData.message} onChange={e => setFormData(prev => ({
                ...prev,
