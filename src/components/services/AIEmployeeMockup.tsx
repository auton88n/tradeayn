...
const AIEmployeeMockup = () => {
  const roles = [
    { icon: Users, label: 'HR', gradient: 'from-blue-400 to-blue-600' },
    { icon: Headphones, label: 'Support', gradient: 'from-emerald-400 to-emerald-600' },
    { icon: TrendingUp, label: 'Sales', gradient: 'from-violet-400 to-violet-600' },
    { icon: Calculator, label: 'Finance', gradient: 'from-amber-400 to-amber-600' },
    { icon: FileText, label: 'Admin', gradient: 'from-orange-400 to-orange-600' },
    { icon: MessageCircle, label: 'Social', gradient: 'from-pink-400 to-pink-600' },
  ];

  const orbitRadius = 120;

  return (
    <div className="relative w-full h-full min-h-[320px] flex items-center justify-center" dir="ltr">
      ...
      <div className="absolute z-20 flex items-center justify-center">
        ...
        {/* Brain container */}
        <motion.div
          className="relative w-20 h-20 rounded-full flex items-center justify-center backdrop-blur-xl"
        ...
        {roles.map((role, index) => {
          ...
          return (
            <motion.div
              ...
              style={{ x, y, marginLeft: -28, marginTop: -28 }}
              initial={{ opacity: 0, scale: 0 }}
              animate={{ opacity: 1, scale: 1, rotate: -360 }}
              transition={{
                opacity: { delay: index * 0.1, duration: 0.5 },
                scale: { delay: index * 0.1, duration: 0.5, type: "spring", stiffness: 200 },
            ...
            >
              ...
              >
                ...
                <div 
                  className={`absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full bg-gradient-to-r ${role.gradient} opacity-80`}
                />
                ...
                {/* Hover glow */}
                <div 
                  className={`absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-br ${role.gradient}`}
