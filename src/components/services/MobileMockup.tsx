...
const MobileMockup = () => {
  ...
  return <div className="relative flex justify-center items-center py-8 gap-8">
      {/* Laptop Frame */}
      ...
      once: true
        ...
        <div className="w-[260px] h-[340px] bg-neutral-800 rounded-[24px] p-3 shadow-2xl">
          ...
          <div className="w-full h-full bg-neutral-950 rounded-[16px] overflow-hidden relative flex flex-col items-center justify-center px-6 pt-4">
            ...
            {/* Subtitle */}
            <motion.p className="text-rose-400/80 text-[9px] tracking-wider uppercase mb-2" initial={{
            opacity: 0
          }} whileInView={{
            opacity: 1
          }} transition={{
            ...
            {/* Bio */}
            <motion.p className="text-neutral-500 text-[8px] text-center leading-relaxed mb-4 px-2 max-w-[200px]" initial={{
            opacity: 0
          }} whileInView={{
            opacity: 1
          }} transition={{
            ...
            <div className="flex gap-2 mb-4">
              {stats.map((stat, i) => <motion.div key={i} className="bg-neutral-800/60 backdrop-blur-sm rounded-lg py-2 px-3 text-center border border-neutral-700/30" initial={{
              opacity: 0,
              y: 10
            }} whileInView={{
              opacity: 1,
      ...
      once: true
        ...
        <div className="w-full h-full bg-neutral-950 rounded-[32px] overflow-hidden relative">
          ...
          <div className="flex flex-col items-center justify-center h-full px-4 pb-6 pt-4 -mt-4 relative z-10">
            ...
            {/* Bio */}
            <motion.p className="text-neutral-500 text-[7px] text-center leading-relaxed mb-4 px-2 max-w-[160px]" initial={{
            opacity: 0
          }} whileInView={{
            opacity: 1
          }} transition={{
            ...
            <div className="flex gap-2 mb-4 w-full px-1">
              {stats.map((stat, i) => <motion.div key={i} className="flex-1 bg-neutral-800/60 backdrop-blur-sm rounded-lg py-2 px-1 text-center border border-neutral-700/30" initial={{
              opacity: 0,
              y: 10
            }} whileInView={{
              opacity: 1,
            ...
            {/* CTA Button */}
            <motion.button className="flex items-center gap-1.5 bg-white text-neutral-900 rounded-full px-4 py-1.5 text-[9px] font-medium shadow-lg mb-4" initial={{
            opacity: 0,
            scale: 0.9
          }} whileInView={{
            opacity: 1,
          ...
          <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-2">
            {navDots.map((_, i) => <motion.div key={i} className={`rounded-full transition-all ${i === 0 ? 'w-1.5 h-1.5 bg-rose-400' : 'w-1 h-1 bg-neutral-600'}`} initial={{
            opacity: 0,
            x: 10
          }} whileInView={{
            opacity: 1,
