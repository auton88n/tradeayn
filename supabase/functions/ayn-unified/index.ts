...
serve(async (req) => {
  ...
  try {
    ...
    // Handle chart analysis intent
    if (intent === 'chart_analysis') {
      try {
        console.log('[ayn-unified] Chart analysis requested');
        
        const fileCtx = context?.fileContext as { name?: string; type?: string; url?: string } | undefined;
        
        if (!fileCtx?.url || !fileCtx?.type?.startsWith('image/')) {
          const noImageMessages: Record<string, string> = {
            ar: '📊 لتحليل شارت التداول، ارفق لقطة شاشة للشارت مع رسالتك.',
            fr: '📊 Pour analyser un graphique, joignez une capture d\'écran du graphique à votre message.',
            en: '📊 To analyze a trading chart, please attach a chart screenshot with your message.'
          };
          return new Response(JSON.stringify({
            content: noImageMessages[language] || noImageMessages.en,
        ...
        // Call analyze-trading-chart edge function internally
        const supabaseUrl = Deno.env.get('SUPABASE_URL')!;
        ...
        if (!chartResponse.ok) {
          ...
          throw new Error(`Chart analysis failed: ${errorText}`);
        }

        const chartResult = await chartResponse.json();

        // Build a markdown summary
        const signal = chartResult.prediction?.signal || 'NEUTRAL';
        const confidence = chartResult.prediction?.confidence || 0;
        const ticker = chartResult.ticker || 'Unknown';
        const timeframe = chartResult.timeframe || 'unknown';
        const reasoning = chartResult.prediction?.reasoning || '';
        const entryZone = chartResult.prediction?.entry_zone || 'N/A';
        const stopLoss = chartResult.prediction?.stop_loss || 'N/A';
        const takeProfit = chartResult.prediction?.take_profit || 'N/A';
        const riskReward = chartResult.prediction?.risk_reward || 'N/A';

        const signalEmoji = signal === 'BULLISH' ? '🟢' : signal === 'BEARISH' ? '🔴' : '🟡';

        const summaryMessages: Record<string, string> = {
          en: `${signalEmoji} **${signal}** signal on **${ticker}** (${timeframe}) — ${confidence}% confidence\n\n${reasoning}\n\n**Trade Setup:**\n- Entry: ${entryZone}\n- Stop Loss: ${stopLoss}\n- Take Profit: ${takeProfit}\n- Risk/Reward: ${riskReward}\n\n⚠️ ${chartResult.disclaimer || 'Not financial advice.'}`,
          ar: `${signalEmoji} إشارة **${signal}** على **${ticker}** (${timeframe}) — ثقة ${confidence}%\n\n${reasoning}\n\n**خطة التداول:**\n- الدخول: ${entryZone}\n- وقف الخسارة: ${stopLoss}\n- جني الأرباح: ${takeProfit}\n- المخاطرة/العائد: ${riskReward}\n\n⚠️ ${chartResult.disclaimer || 'ليست نصيحة مالية.'}`,
          fr: `${signalEmoji} Signal **${signal}** sur **${ticker}** (${timeframe}) — ${confidence}% de confiance\n\n${reasoning}\n\n**Setup de trading:**\n- Entrée: ${entryZone}\n- Stop Loss: ${stopLoss}\n- Take Profit: ${takeProfit}\n- Risk/Reward: ${riskReward}\n\n⚠️ ${chartResult.disclaimer || 'Pas un conseil financier.'}`
        };
        ...
        try {
          await supabase.from('llm_usage_logs').insert({
            user_id: userId,
            intent_type: 'chart_analysis',
            was_fallback: false
          });
        } catch (logError) {
          console.error('Failed to log chart analysis usage:', logError);
        }

        return new Response(JSON.stringify({
          content: summaryMessages[language] || summaryMessages.en,
        ...
        console.error('[ayn-unified] Chart analysis failed:', chartError);
        const errorMessages: Record<string, string> = {
          ar: 'عذراً، حدث خطأ أثناء تحليل الشارت. حاول مرة أخرى؟',
          fr: "Désolé, une erreur est survenue lors de l'analyse du graphique. Réessayer?",
          en: "Sorry, couldn't analyze that chart right now. Try again?"
        };
        return new Response(JSON.stringify({
          content: errorMessages[language] || errorMessages.en,
          error: chartError instanceof Error ? chartError.message : 'Chart analysis failed',
          intent: 'chart_analysis'
        }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
